#!/usr/bin/env python3
"""
Batch Preprocessing Pipeline - Process in chunks to avoid memory issues
"""
...
def process_batch(ecg_data_batch: dict, reference_batch: pd.DataFrame, 
    ...
    for idx, row in reference_batch.iterrows():
        ...
        if patient_id in ecg_data_batch:
            ...
            processed_signal = preprocessor.preprocess_single(ecg_signal)
...
def batch_preprocess_pipeline(config_path: str = "config/config.yaml", batch_size: int = 5000):
    ...
    logger.info("\\n" + "="*60)
    logger.info("STEP 3: Batch Processing ECG Signals")
    logger.info("="*60)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Batch ECG Preprocessing")
